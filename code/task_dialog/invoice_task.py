...
def invoice_judge(sentence):
    ...
    if True:
        ...
        if re.search("帮|要|怎样|怎么|如何|哪.*发票|哪里|开.*票", sentence):
            ...
            if re.search("纸质发票",
                         sentence): return "您好,下单时选择“普通发票（纸质）”自助开取，此发票可用作单位报销凭证，一个订单对应一张或多张发票，不同的物流中心发出的包裹开具不同的发票，发票会随每次包裹一同发出。"
            ...
            if re.search("增票|增值税发票|增税发票|专票|专用发票", sentence):
                if re.search("审核",
                             sentence): return "我们会在[数字x]个工作日审核,审核通过后您下单选择对应的增票信息即可开具啦,#E-s[数字x]#E-s[数字x]#E-s[数字x],请问还有其他还可以帮到您的吗#E-s[数字x]"
