...
### 如何接入SMN以及其他多轮模型
因为比赛告一段落，不再有继续改进这个项目的源动力。

SMN模型以及其他多轮模型运行后应该生成"test_result.tsv"文件，然后放置在bert/out文件夹下，调用bert的result estimator即可评测

## score in Online Judge
1. 0.0618: 10per skipgram 无停用词 k = 30 中心  只输入q 不替换
